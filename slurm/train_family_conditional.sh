#!/bin/bash
#SBATCH --job-name=dima-family-cfg-650m
...
#SBATCH --output=logs/dima_family_cfg_%j.out
#SBATCH --error=logs/dima_family_cfg_%j.err

# ============================================================
# Family-Specific Conditional Fine-tuning - ESM-650M on Leonardo HPC
# §3.6.2: Conditional Fine-tuning approach
# ============================================================
...
mkdir -p $WORK_DIR/generated_sequences/family
...
echo "Starting family-specific conditional fine-tuning with ESM-650M..."
...
torchrun \
    ...
    pixi run -e dima-env python train_family.py \
        --config-name config_family \
        ...
        project.wandb_project=dima-family-cfg \
        ddp.enabled=true \
        training.training_iters=2500 \
        training.eval_interval=500 \
        training.save_interval=500 \
        training.batch_size=1024 \
        training.lr=0.00005 \
        conditional.approach=conditional_finetuning
